...
const episodes = defineCollection({
  ...
  schema: z.object({
    ...
    status: z.enum(['upcoming', 'live']).default('live'),
    episodeType: z.enum(['full', 'trailer', 'bonus']).default('full'),
    durationSeconds: z.number().optional(),
    durationMinutes: z.number().optional(),
    guestName: z.string().optional(),

    // Synced from the Transistor RSS feed by scripts/sync-transistor.py.
    audioUrl: z.string().optional(),
    audioBytes: z.number().optional(),
    transistorShareUrl: z.string().optional(),
    transistorEmbedUrl: z.string().optional(),
    artworkUrl: z.string().optional(),
    guid: z.string().optional(),
    explicit: z.boolean().default(false),
    keywords: z.array(z.string()).default([]),

    // Hand-curated (LOCAL_FIELDS in the sync script — these survive a re-sync).
